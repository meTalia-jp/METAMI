...
- サムネイルを大きく表示する「サムネイルカード」一覧
- 小さなサムネイルとタイトル、評価、タグ、メモ、ファイル情報を
  横長に表示する「ランドスケープカード」一覧
- 2種類のカード表示の切り替え、保存、再起動後の復元
- 縦長・横長・1:1素材に応じたカード表示
- `blake2b_256`によるファイル識別情報の保存基盤と、未計算・計算済み・
  再計算必要などのハッシュ状態管理
- 新しく登録した画像のファイル識別情報を初期設定で自動登録し、設定から
  自動登録を無効化可能
- 500件以上の新規画像では開始前に確認し、画像は登録を見送った場合も一覧へ追加
- 選択ファイルの手動登録・再登録と、画像・動画・サブフォルダを選べる
  フォルダ単位の一括登録
- バックグラウンドでの登録、進捗表示、キャンセル、登録状況の確認
- 同じ内容のファイルに以前のMETAMIデータがある場合の候補通知と参照
- 現在のフォルダ内にあるコピー可能なファイルへのMETAMIデータ一括コピー
- 画像プレビューとMP4再生
...
- METAMIデータの手動バックアップ・復元
- データ保存場所の表示
- DBスキーマの互換性確認、更新前バックアップ、新しいDBの安全な保護
- データベース情報画面でのDBスキーマ確認
- 登録場所で見つからないファイルの記録表示と、利用者確認によるDB記録整理
...
### 同じ内容のファイルからMETAMIデータを再利用

ファイル名や保存場所が異なる場合でも、ファイル識別情報を使って同じ内容の
ファイルを確認できます。以前の登録にメモ・タグ・星評価・利用者タイトルが
ある場合は候補を確認し、現在開いているフォルダ内のコピー可能なファイルへ
まとめてコピーできます。

すでにMETAMIデータがあるファイルは上書きしません。異なるデータ候補が複数
ある場合も自動コピーしません。コピー後は元の登録とコピー先をそれぞれ独立
したMETAMIデータとして扱い、自動同期は行いません。
...
- 「登録パスを変更」: 利用者が新しいファイルを1件選び、確認後に既存の
  DB記録の登録パスを変更します。フォルダやドライブの自動検索ではありません。
...
- ファイル識別情報を使ったMETAMIデータのコピーは、現在開いているフォルダ
  単位です。ファイルごと・項目ごとの選択、既存データへの上書き、異なる候補の
  自動選択、自動追跡、自動同期、重複整理、miniDBには対応していません。
- Ver1.0.8以前からMETAMIへ登録されている画像のファイル識別情報は、自動では
  登録されません。「データ管理」→「ファイル識別情報」から手動でまとめて
  登録できます。更新時の大量自動処理を避けるための仕様です。
